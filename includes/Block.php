...
final class Block {
	...
	public static function register_blocks() {
		...
		$blocks = apply_filters( 'saucal_hub_gutenberg_blocks', array() );
	...
	public static function register( $block = '', $args = array() ) {
		...
		// Set editor styles.
		$editor_style = 'saucal-hub-' . $block_name . '-blocks';
		if ( ! empty( $args['editor_style'] ) ) {
			$editor_style = $args['editor_style'];
		}

		register_block_type(
			'saucal-hub/' . $block_name,
			array(
				'style'         => $styles,
				'script'        => $script,
				'editor_style'  => $editor_style,
				'editor_script' => 'saucal-hub-' . $block_name . '-blocks',
			)
		);
	}
}
